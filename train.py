...
def train_fn(cfgs: TotalConfigs, model_name: str, model: nn.Module, matcher: HungarianMatcher, train_loader, valid_loader, device):
    optimizer = optim.Adam(model.parameters(), lr=cfgs.train.learning_rate, weight_decay=cfgs.weight_decay)
    ...
    # MSVD datasets start
    # idx2word: {0: '<EOS>', 1: '<SOS>', 2: '<UNK>', 3: 'a', 4: 'bird', 5: 'in', ...}
    # vid2groundtruth['mv89psg6zh4_33_46'][frame_index]: 'a bird in a sink keeps getting under the running water from a faucet'
    ...
    logger.info(cfgs.train.save_checkpoints_path)
    for epoch in range(cfgs.train.max_epochs):
        ...
        for i, (feature2ds, feature3ds, objects, object_masks, vp_semantics, caption_semantics, numberic_caps, masks, captions, nouns, vids, vocab_ids, vocab_probs, fillmasks) in enumerate(train_loader):
            ...
            if cnt % cfgs.train.save_checkpoints_every == 0:
                ...
                if epoch > cfgs.train.max_epochs-2:
                    torch.save(model.state_dict(), 
                               os.path.join(cfgs.train.checkpoints_dir, 'epochs_{current_epoch}cider_score{cider_score}.pth'
                                            .format(current_epoch = epoch, cider_score = cider_score))
                    )
                print('=' * 10,
                      '[EPOCH{epoch} iter{it}] :Best Cider is {bs}, Current Cider is {cs}'.
                      format(epoch=epoch, it=i, bs=best_score, cs=cider_score),
                      '=' * 10)
                logger.info('[EPOCH{epoch} iter{it}] :Best Cider is {bs}, Current Cider is {cs}'.
                      format(epoch=epoch, it=i, bs=best_score, cs=cider_score))
