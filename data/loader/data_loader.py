...
def get_ids_and_probs(fillmask_steps, max_caption_len):
    ...
    # fill zero into the ret_ids to pad the rest of the max_caption_len
    for i in range(gap):
...
class CaptionDataset(Dataset):
    def __init__(self, cfgs: TotalConfigs, mode, save_on_disk=False, is_total=False):
        ...
        # print(type(self.video_ids)) : <class 'list'>  example:MSRVTT
        ...
        with h5py.File(backbone2d_path, 'r') as f:
            for vid in video_ids:
                # temp_feat.shape : (57, 1536)
                temp_feat = f[vid][()]
                # from 0 to len(temp_feat) - 1 split to sample_numb pieces averagely
                # from temp_feat frames choose sample_numb frames and then store it to the self.backbone_2d_dict
                sampled_idxs = np.linspace(0, len(temp_feat) - 1, sample_numb, dtype=int)
                # self.backbone_2d_dict[vid].shape : (15, 1536)
                self.backbone_2d_dict[vid] = temp_feat[sampled_idxs]

        # feature 3d dict
        with h5py.File(backbone3d_path, 'r') as f:
            for vid in video_ids:
                # temp_feat.shape : (23, 2048)
                temp_feat = f[vid][()]
                sampled_idxs = np.linspace(0, len(temp_feat) - 1, sample_numb, dtype=int)
                # self.backbone_3d_dict[vid].shape : (15, 2048)
        ...
        with h5py.File(objects_path, 'r') as f:
            for vid in video_ids:
                ...
                # temp_feat = f[vid][()]
                # self.objects_dict[vid].shape : (46, 2048)
                # object features : (frames, channels)
                self.objects_dict[vid] = temp_feat

        with open(vid2language_path, 'rb') as f:
            # len(self.vid2language['video0']) : 20, the choosen frames of video0 is 20
            # type(self.vid2language['video0'][0]) : <class 'tuple'>
            # self.vid2language['video0'][0][0] : 'a car is shown' : [video_id][frame_id][description]
            self.vid2language = pickle.load(f)
        
        if cfgs.train.lambda_soft > 0 and not save_on_disk:
            with open(vid2fillmask_path, 'rb') as f:
                # self.vid2fillmask['video0']['a car is shown'...]
