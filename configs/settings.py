...
def _settings():
    ...
    parser.add_argument('--model_name', type=str, default='EDS', help='which model you would like to train/test?')
    parser.add_argument('--device', type=str, default='cuda:0', help='cuda device')
    # add
    parser.add_argument('--use_SSG', action='store_true', default=False, help='whether use SSG enhence semantic features')
    parser.add_argument('--use_SSE', action='store_true', default=False, help='whether use SSE during training and test process')
    parser.add_argument('--use_ISE', action='store_true', default=False, help='whether use ISE in decoder')
    ...
    parser.add_argument('--backbone_2d_name', type=str, default='inceptionresnetv2', help='2d backbone name (InceptionResNetV2)')
    parser.add_argument('--backbone_3d_name', type=str, default='C3D', help='3d backbone name (C3D)')
    parser.add_argument('--object_name', type=str, default='vg_objects', help='object features name (vg_objects)')
    parser.add_argument('--semantics_dim', type=int, default=768, help='semantics embedding dim')

    """
    =========================Encoder Settings===========================
    """
    parser.add_argument('--backbone_2d_dim', type=int, default=1536, help='dimention for inceptionresnetv2')
    ...
    parser.add_argument('--top_k', type=int, default=5, help='select the top-k similar sentence semantics')
    ...
    parser.add_argument('--num_hidden_layers_decoder', type=int, default=1)
    parser.add_argument('--num_attention_heads', type=int, default=8)
    parser.add_argument('--intermediate_size', type=int, default=2024)
    ...
    parser.add_argument('--layer_norm_eps', type=float, default=0.00001)
    parser.add_argument('--watch', type=int, default=0)
    parser.add_argument('--pos_attention', default=False, action='store_true')
    parser.add_argument('--enhance_input', type=int, default=2, 
    ...
    parser.add_argument('-tie', '--tie_weights', default=False, action='store_true', 
    ...
    parser.add_argument('--save_checkpoints_every', type=int, default=50)
    parser.add_argument('--save_checkpoints_path', type=str, default='-1')
    # add
    parser.add_argument('--file_path', type=str, default='-1')
    ...
    # add
    parser.add_argument('--beam_alpha', type=float, default=0.5,
                            help='the preference of sentence length, larger --> longer')
    parser.add_argument('--base_cider', type=float, default=0)
    parser.add_argument('--topk', type=int, default=5, help='visualize the topk beam searches')

    parser.add_argument('--msg', type=str, default='_top_k_3_1e-4')
...
class TotalConfigs:
    def __init__(self, args):
        ...
        self.msg = args.msg
        self.use_SSE = args.use_SSE
        self.use_SSG = args.use_SSG
...
class DecoderConfigs:
    def __init__(self, args):
        ...
        # add
        self.num_hidden_layers_decoder = args.num_hidden_layers_decoder
        ...
        self.max_len = args.max_caption_len
        self.layer_norm_eps = args.layer_norm_eps
        self.watch = args.watch
        self.pos_attention = args.pos_attention
        self.enhance_input = args.enhance_input
        self.with_layernorm = args.with_layernorm
        self.encoder_dropout = args.encoder_dropout
        self.no_encoder_bn = args.no_encoder_bn
        self.norm_type = args.norm_type
        self.dim_word = args.dim_word
        self.tie = args.tie_weights

        self.embedding_weights_path = os.path.join(args.data_dir, args.dataset_name, 'language', 'embedding_weights.pkl')
        self.n_vocab = -1
        # add
        self.use_ISE = args.use_ISE
...
class TrainingConfigs:
    def __init__(self, args):
        ...
        if args.seed == 1:
            self.checkpoints_dir = os.path.join(args.checkpoints_dir, args.dataset_name + str(args.seed) + args.msg)
        else:
            self.checkpoints_dir = os.path.join(args.checkpoints_dir, args.dataset_name + str(args.seed) + args.msg)
...
class TestConfigs:
    def __init__(self, args):
        ...
        if args.seed == 1:
            ...
            self.result_dir = os.path.join('./results/{dataset_name}'.format(dataset_name=args.dataset_name + str(args.seed)))
        ...
        # add
        self.base_cider = args.base_cider
        self.topk = args.topk
