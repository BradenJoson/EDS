...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, return_pos=False):
        ...
        with open(config.embedding_weights_path, 'rb') as f:
            word_embedding_weights = pickle.load(f)
        self.word_embeddings = nn.Embedding.from_pretrained(torch.FloatTensor(word_embedding_weights),
                                                      freeze=False, padding_idx=Constants.PAD)
        self.word_embeddings_prj = nn.Linear(300, config.hidden_dim)
        self.position_embeddings = PositionalEmbedding(config.max_len, config.hidden_dim)
        self.category_embeddings = None
        self.return_pos = return_pos

        self.LayerNorm = BertLayerNorm(config.hidden_dim, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if self.return_pos:
            self.pos_LN = BertLayerNorm(config.hidden_dim, eps=config.layer_norm_eps)
    ...
    def linear(self, x):
        x = x.matmul(self.word_embeddings.weight.t()) # [batch_size, n_vocab]
        return x
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, attend_to_enc_output=False):
        ...
        if attend_to_enc_output and getattr(config, 'modality_wise_dimensions', False):
            self.key = nn.Linear(config.hidden_dim * len(config.modality), self.all_head_size)
            self.value = nn.Linear(config.hidden_dim * len(config.modality), self.all_head_size)
        else:
    ...
    def forward(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = BertLayerNorm(config.hidden_dim, eps=config.layer_norm_eps) if config.with_layernorm else None
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, non_pad_mask=None, attention_mask=None, 
        ...
        output_attentions=False, head_mask=None):
        all_attentions = ()
