...
class PredicateLevelEncoder(nn.Module):
    def __init__(self, feature3d_dim, hidden_dim, semantics_dim, useless_objects, use_SSG=True):
        super(PredicateLevelEncoder, self).__init__()

        self.linear_layer = nn.Linear(feature3d_dim, hidden_dim)
        self.W = nn.Linear(hidden_dim, hidden_dim)
        self.U = nn.Linear(hidden_dim, hidden_dim)
        self.b = nn.Parameter(torch.ones(hidden_dim), requires_grad=True)  # init b with torch.one(512) 
        ...
        self.use_SSG = use_SSG
        if not self.use_SSG:
            self.fc_layer = nn.Linear(hidden_dim, semantics_dim)
        else:
            self.fc_layer = nn.Linear(hidden_dim + semantics_dim, semantics_dim)


    def forward(self, features3d: Tensor, objects: Tensor, objects_mask: Tensor, objects_semantics: Tensor):
        """
        ...
        action = torch.max(output, dim=1)[0]
        if not self.use_SSG:
            action_pending = self.fc_layer(action)  # (bsz, semantics_dim)
        else:
            objects_semantics = torch.mean(objects_semantics, dim=1)
            action_pending = self.fc_layer(torch.cat([action, objects_semantics], dim=-1))  # (bsz, semantics_dim)
