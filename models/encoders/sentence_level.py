...
class SentenceLevelEncoder(nn.Module):
    def __init__(self, feature2d_dim, hidden_dim, semantics_dim, useless_objects, use_SSG=True):
        ...
        if not self.use_SSG:
            ...
            self.fc_layer = nn.Linear(hidden_dim + 2 * semantics_dim, semantics_dim)
        
    def forward(self, feature2ds: Tensor, vp_features: Tensor, object_features: Tensor, objects_mask: Tensor, objects_semantics: Tensor, action_semantics: Tensor):
        """
        ...
        if not self.use_SSG:
            ...
            video_pending = self.fc_layer(torch.cat([video, objects_semantics, action_semantics], dim=-1))  # (bsz, semantics_dim)  
