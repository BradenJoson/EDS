...
from utils.utils import auto_enlarge


class Translator(object):
    ''' Load with trained model and handle the beam search '''

    def __init__(self, opt, model, device, teacher_model=None, dict_mapping={}):
        self.opt = opt
        self.model = model
        self.device = device
    ...
    def prepare_inputs_for_decoder(self, encoder_outputs, category):
        if category is not None:
            inputs_for_decoder = {'enc_output': encoder_outputs, 'category': category}
        else:
            inputs_for_decoder = {'enc_output': encoder_outputs}
    ...
    def translate_batch_ARFormer(self, encoder_outputs, category):
        ...
        with torch.no_grad():
            inputs_for_decoder = self.prepare_inputs_for_decoder(encoder_outputs, category)
            ...
            #-- Prepare beams
            inst_dec_beams = [Beam(n_bm, self.opt["max_caption_len"], device=self.device, specific_nums_of_sents=self.opt.get('topk', 1)) for _ in range(n_inst)]
            ...
            #-- Decode
            for len_dec_seq in range(1, self.opt["max_caption_len"]):
